...
4. You will spawn parallel agents to implement logics, frontend, views, tasks, ... . Don't forget to use `frontend-craftsman`, `django-view-architect`, and `celery-architect` agents for them!
5. After implementation is done, write unit tests covering the new logic. In parallel you run mypy strict mode for any errors and ask `uncle bob` for any feedback on code
