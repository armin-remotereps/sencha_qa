...
from omniparser.services.parser import (
    OmniParserService,
    RawElementDict,
    _build_draw_config,
    _build_element,
...
from omniparser.types import BBox, ParseResult, PixelParseResult, UIElement
...
class OmniParserServiceParseTest(TestCase):
    ...
    @patch("omniparser.services.parser._ensure_omniparser_on_path")
    def test_parse_returns_parse_result(self, mock_path: MagicMock) -> None:
        ...
        with (
            patch(
                "omniparser.services.parser._decode_image",
                return_value=(mock_image, 1920, 1080),
            ),
            patch(
                "omniparser.services.parser._run_ocr",
                return_value=("text", [[0.1, 0.2, 0.3, 0.4]]),
            ),
            patch(
                "omniparser.services.parser._run_som_labeling",
